...
import { integrationsQueryOptions } from '#/lib/integrations'

export const Route = createFileRoute('/_authed/settings/companies/$companyId/accounts')({
...
function AccountsSection() {
  ...
  const companies = useQuery(companiesQueryOptions(api, { includeInactive: true }))
  const integrations = useQuery(integrationsQueryOptions(api))

  const company = companies.data?.find((c) => c.id === companyId)
  // The company's first connected integration — the same one the edit dialog
  // manages, so the two views never disagree about which connection is "the" one.
  ...
  const update = useMutation(updateAccountMutation(api, queryClient, integration?.id ?? ''))
  const refresh = useMutation(refreshAccountsMutation(api, queryClient, integration?.id ?? ''))

  if (companies.isError || integrations.isError) {
    return (
      <Card className="p-8 text-center">
        <h2 className="font-display text-base font-medium">Couldn’t load this company</h2>
  ...
  if (!companies.isPending && company === undefined) {
    return (
      <Card className="p-8 text-center">
        <h2 className="font-display text-base font-medium">Company not found</h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
  ...
  return (
    <AccountsPanel
      ...
      loading={companies.isPending || integrations.isPending || accounts.isPending}
      canManage={canManage}
