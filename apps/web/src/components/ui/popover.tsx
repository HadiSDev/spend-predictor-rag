...
export const PopoverContent = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<typeof BasePopover.Popup> & { sideOffset?: number; align?: 'start' | 'center' | 'end' }
>(({ className, children, sideOffset = 8, align = 'center', ...props }, ref) => (
  <BasePopover.Portal>
    <BasePopover.Positioner className="z-50 outline-none" sideOffset={sideOffset} align={align}>
      <BasePopover.Popup
        ref={ref}
        className={cn(
...
>(({ className, ...props }, ref) => (
  <BasePopover.Title ref={ref} className={cn('text-sm font-medium', className)} {...props} />
))
...
>(({ className, ...props }, ref) => (
  <BasePopover.Description ref={ref} className={cn('text-sm text-muted-foreground', className)} {...props} />
))
