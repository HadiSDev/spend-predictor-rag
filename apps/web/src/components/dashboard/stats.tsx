...
import type { EntrySummaryRow } from '#/lib/types'

/** Sum net per currency (never across currencies) from entries-summary rows. */
function netByCurrency(rows: Array<EntrySummaryRow>): Array<{ currency: string | null; net: number }> {
  const totals = new Map<string | null, number>()
  for (const row of rows) {
    totals.set(row.currency, (totals.get(row.currency) ?? 0) + toNumber(row.net))
  }
...
export function DashboardStats({ rows }: { rows: Array<EntrySummaryRow> }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <StatCard icon={<Activity />} label="Transactions" value={formatCount(totalCount)} />
      <StatCard icon={<Layers />} label="Entry types" value={formatCount(entryTypes)} />
    </div>
  )
}
