...
import { postingAmount } from '#/lib/format/entry-amount'
import { formatMoney, toNumber } from '#/lib/format/format'
import type { ErpEntryRead } from '#/lib/api/types'
import { wasConverted } from '#/components/entries/converted-amount'

/** A labelled field in a posting's detail. */
export function Row({
  label,
  children,
}: {
...
/** The currency conversion of a posting, if any. */
    ...
    <>
      ...
      <Row label="Exchange rate">
        <span className="tabular-nums">
          {toNumber(entry.fx_rate ?? 0).toLocaleString('en-GB', {
...
/** One posting's signed amount, as posted. */
...
/** One posting, collapsed to its account and amount until opened. */
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="font-medium tabular-nums">
            {entry.erp_account_code}
          </span>
          <span className="truncate text-muted-foreground">
            {entry.erp_account_name}
          </span>
          {entry.status === 'failed' ? (
      ...
      {open ? (
        <dl className="divide-y divide-border bg-muted/30 px-4">
          ...
          <Row label="Debit">
            <Value>
              {entry.debit_amount
                ? formatMoney(entry.debit_amount, entry.currency)
                : null}
            </Value>
          </Row>
          <Row label="Credit">
            <Value>
              {entry.credit_amount
                ? formatMoney(entry.credit_amount, entry.currency)
                : null}
            </Value>
          </Row>
          <Row label="Currency">
          ...
          <Row label="Status">
            <Badge
              variant={entry.status === 'failed' ? 'destructive' : 'default'}
            >
              {entry.status}
            </Badge>
          </Row>
...
/** The Postings tab: every raw GL row of the voucher. */
