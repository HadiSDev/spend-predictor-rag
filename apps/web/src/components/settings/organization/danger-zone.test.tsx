...
const confirmButton = () =>
  screen
    .getAllByRole('button', { name: 'Suspend organization' })
...
describe('DangerZone', () => {
  ...
  it('surfaces a failure without closing the dialog', async () => {
    ...
    render(
      <DangerZone organizationName="Acme" canManage onSuspend={onSuspend} />,
    )
    fireEvent.click(
      screen.getByRole('button', { name: 'Suspend organization' }),
    )
  ...
  it('states that data is retained', () => {
    ...
    expect(
      screen.getByText(/retains all of this organization’s data/),
    ).toBeTruthy()
  })
})
