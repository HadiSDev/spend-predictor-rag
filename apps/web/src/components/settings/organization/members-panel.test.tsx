...
describe('MembersView', () => {
  ...
  it('shows the role label in the select trigger, not the Clerk key', () => {
    ...
    expect(
      screen.getByLabelText('Role for grace@example.com').textContent,
    ).toBe('Member')
  ...
  it('sends an invitation and clears the field', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByLabelText<HTMLInputElement>('Invite email address').value,
      ).toBe(''),
    )
  })
  ...
  it('explains what to do when the Clerk instance has invitations disabled', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/aren’t enabled for this Clerk instance/),
      ).toBeTruthy(),
    )
  ...
  it('surfaces a rejected invitation on the form', async () => {
    const onInvite = vi.fn().mockRejectedValue({
      errors: [
        { longMessage: 'That user is already a member of this organization.' },
      ],
    })
    renderView({ onInvite })
    ...
    await waitFor(() =>
      expect(
        screen.getByText('That user is already a member of this organization.'),
      ).toBeTruthy(),
    )
  })
