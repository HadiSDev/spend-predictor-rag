...
/** Providers offered for connecting. */
const PROVIDERS = [
...
export interface SecurityViewProps {
  /** Whether a password already exists. */
  hasPassword: boolean
...
export function SecurityView({
  ...
  const visibleSessions = sessions.filter(
    (session) => !revoked.includes(session.id),
  )
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="flex flex-col gap-4">
              ...
              {connections.map((connection) => (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void handleConnection(() => onDisconnect(connection.id))
                    }
                  >
                    Disconnect
                  </Button>
          ...
          {!canDisconnect ? (
            <p className="text-sm text-muted-foreground">
              This is your only way to sign in. Set a password before
              disconnecting it.
            </p>
          ) : null}
          ...
          <div className="flex flex-wrap gap-2">
            {PROVIDERS.filter(
              (provider) => !connected.has(provider.provider),
            ).map((provider) => (
              <Button
                ...
                disabled={busy}
                onClick={() =>
                  void handleConnection(() => onConnect(provider.strategy))
                }
              >
...
export function SecurityPanel({ user }: { user: ClerkUser }) {
  ...
  const connections: Array<ConnectionRow> = user.externalAccounts.map(
    (account) => ({
      id: account.id,
