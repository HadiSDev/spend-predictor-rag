...
import type { ErpAccountRead } from '#/lib/api/types'
...
function props(
  overrides: Partial<AccountsPanelProps> = {},
): AccountsPanelProps {
  return {
...
describe('AccountsPanel', () => {
  ...
  it('invites a refresh when the chart has never been fetched', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Refresh from ERP/ }),
    ).toBeTruthy()
  })
  ...
  it('disables every write control for a non-manager', () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Refresh from ERP/ })
        .hasAttribute('disabled'),
    ).toBe(true)
    expect(
      screen
        .getByRole('button', { name: 'Enable 3 shown' })
        .hasAttribute('disabled'),
    ).toBe(true)
  ...
  it('shows placeholders while loading', () => {
    const { container } = render(
      <AccountsPanel {...props({ loading: true })} />,
    )
    expect(
