...
import type { SpendCategoryRead, SpendTreeDetailRead } from '#/lib/api/types'
...
function renderEditor(
  overrides: Partial<React.ComponentProps<typeof SpendTreeEditor>> = {},
) {
  const props = {
    tree: tree(),
    canManage: true,
...
describe('SpendTreeEditor', () => {
  it('renders the hierarchy, not a flat list of level columns', () => {
    ...
    expect(
      screen.getByRole('button', { name: /expand technology/i }),
    ).toBeTruthy()
  })
  ...
  it('expands and collapses a branch', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /collapse technology/i }),
    )
  ...
  it('adds a child under a node', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /add a category under technology/i }),
    )
  ...
  it('makes the depth limit visible before it bites', () => {
    ...
    const add = screen.getByRole('button', {
      name: /add a category under cloud/i,
    })
  ...
  it('refuses to delete a node with children, and says why', async () => {
    ...
    expect(screen.getByRole('button', { name: /^delete$/i })).toHaveProperty(
      'disabled',
      true,
    )
  ...
  it('warns that assigned lines will need reviewing before deleting a leaf', async () => {
    ...
    expect(
      await screen.findByText(
        /keep the category on record but will need reviewing/i,
      ),
    ).toBeTruthy()
  ...
  it('withholds every mutating control from a read-only member', () => {
    ...
    expect(
      screen.getByRole('button', { name: /top-level category/i }),
    ).toHaveProperty('disabled', true)
    expect(
      screen.getByRole('button', { name: /edit technology/i }),
    ).toHaveProperty('disabled', true)
    expect(
      screen.getByRole('button', { name: /delete technology/i }),
    ).toHaveProperty('disabled', true)
  })
