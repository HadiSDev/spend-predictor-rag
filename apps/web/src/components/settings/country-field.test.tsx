...
function renderField(value = '') {
  ...
  return { onChange, input: screen.getByRole<HTMLInputElement>('combobox', { name: 'Country' }) }
}
...
describe('CountryField', () => {
  ...
  it('resolves a code saved by the free-text field it replaced', () => {
    // The whole reason the lookup is case-insensitive: `dk` was expressible.
    const { input } = renderField('dk')
  ...
  it('finds a country by code as readily as by name', async () => {
    ...
    expect(await screen.findByRole('option', { name: 'NL — Netherlands' })).toBeTruthy()
  })
  ...
  it('finds an accented name typed without the accent', async () => {
    ...
    expect(await screen.findByRole('option', { name: 'CW — Curaçao' })).toBeTruthy()
  })
  ...
  it('offers the whole list, not just the currency-issuing countries', async () => {
    ...
    // Kenya has no currency in `CURRENCIES` and no flag asset — it must still
    // be selectable, which is the point of carrying all 249.
    expect(await screen.findByRole('option', { name: 'KE — Kenya' })).toBeTruthy()
  })
})
