...
import type { SpendTreeRead } from '#/lib/types'
...
function renderPanel(overrides: Partial<SpendTreesPanelProps> = {}) {
  const props: SpendTreesPanelProps = {
    ...
    onImport: vi.fn().mockResolvedValue({ created: 4, updated: 0, removed: 0, stale_lines: 0 }),
    onOpenTree: vi.fn(),
    ...overrides,
  }
...
describe('SpendTreesPanel', () => {
  it('lists each tree with what a manager chooses between them on', () => {
    ...
    expect(screen.getByText(/3 levels · 24 categories · Used by Acme A\/S/)).toBeTruthy()
  ...
  it('clones an existing tree', async () => {
    ...
    fireEvent.change(await screen.findByLabelText('Name'), { target: { value: 'Our tree' } })
  ...
  it('creates an empty tree with no source', async () => {
    ...
    fireEvent.change(await screen.findByLabelText('Name'), { target: { value: 'Blank' } })
  ...
  it('shows the row count before an import is applied', async () => {
    ...
    fireEvent.change(screen.getByLabelText('CSV'), {
      target: { value: 'level_1,level_2\nIndirect,Technology\nIndirect,Logistics\n' },
    })
    // Nobody should load a file blind.
    expect(screen.getByText(/3 lines to import/i)).toBeTruthy()
  })

  it('names the offending rows when an import is rejected', async () => {
    // A user fixes a spreadsheet by going to a line; "the import failed" sends
    // them nowhere.
    ...
    fireEvent.click(screen.getByRole('button', { name: /new tree/i }))
    fireEvent.change(await screen.findByLabelText('Name'), { target: { value: 'Imported' } })
...
describe('SpendTreesPanel — the default tree', () => {
  it('offers to add the default when the organization has none', async () => {
    // Copy-on-first-use is triggered only by creating a company, so an
    // organization that predates spend trees would otherwise have no way to
    // obtain the default at all.
    const props = renderPanel({ trees: [tree({ source: 'custom', name: 'Test' })] })
    fireEvent.click(screen.getByRole('button', { name: /add the default tree/i }))
  ...
  it('offers it from the empty state too', async () => {
    ...
    expect(screen.getByText(/companies created before spend trees existed have none/i))
      .toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: /add the default tree/i }))
    await waitFor(() => expect(props.onUseDefault).toHaveBeenCalled())
  })
  ...
  it('stops offering it once the copy exists', () => {
    ...
    expect(screen.queryByRole('button', { name: /add the default tree/i })).toBeNull()
  })

  it('withholds it from a read-only member', () => {
    renderPanel({ trees: [], canManage: false })
    expect(screen.getByRole('button', { name: /add the default tree/i })).toHaveProperty(
      'disabled',
      true,
    )
  })
...
describe('SpendTreesPanel — deleting a tree', () => {
  it('deletes an unused tree from the row menu', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /actions for default spend tree/i }))
  ...
  it('surfaces a refusal and lets a line-orphaning delete be confirmed', async () => {
    // Two distinct 409s: one is retryable with a confirmation, one is not.
    const onDelete = vi
      .fn()
    ...
    expect(await screen.findByText(/3 invoice line\(s\) are categorized/i)).toBeTruthy()
  ...
  it('keeps a tree that is still in use, showing the reason', async () => {
    const onDelete = vi.fn().mockRejectedValue({
      detail: 'This tree is in use by Acme A/S. Assign those companies another tree first.',
    })
    renderPanel({ onDelete })
    ...
    // Not retryable by confirming — the fix is to reassign the company.
