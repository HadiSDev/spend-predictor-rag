...
function renderPanel(overrides: Partial<Parameters<typeof OrganizationPanel>[0]> = {}) {
...
describe('OrganizationPanel', () => {
  it('lets an admin edit the profile', async () => {
    ...
    await waitFor(() => expect(onSave).toHaveBeenCalledWith({ name: 'Acme Group', slug: 'acme' }))
  })
  ...
  it('puts a 409 on the slug field and keeps the form editable', async () => {
    const onSave = vi
      .fn()
      .mockRejectedValue(new ApiError(409, 'generic', { detail: 'Slug already in use' }))
    renderPanel({ onSave })
