...
function setup(props: Partial<React.ComponentProps<typeof TreeSuggestions>> = {}) {
  return render(
    <TreeSuggestions
...
describe('TreeSuggestions', () => {
  it('names the proposal and where it would go', () => {
    ...
    expect(screen.getByText(/under Indirect > Travel & Entertainment/)).toBeTruthy()
  })
  ...
  it('renders nothing when there is nothing pending', () => {
    // An empty panel with a cheerful message is apparatus the reader has to
    // learn to ignore, on a page that is about editing a taxonomy.
  ...
  it('ignores suggestions that are already settled', () => {
    const { container } = setup({
      suggestions: [suggestion({ state: 'dismissed' }), suggestion({ id: 's2', state: 'accepted' })],
    })
  ...
  describe('acting on one', () => {
    ...
    it('will not accept one whose parent is gone', () => {
      setup({ suggestions: [suggestion({ acceptable: false, parent_path: null })] })

      expect(screen.getByRole('button', { name: /add to tree/i })).toHaveProperty('disabled', true)
      expect(screen.getByText(/parent no longer exists/i)).toBeTruthy()
    })

    it('offers a real undo after a dismissal', async () => {
      // Dismiss sits one click from accept, so a misclick must not permanently
      // lose a proposal the customer wanted.
      const onReopen = vi.fn(() => Promise.resolve())
      setup({ onReopen })

      fireEvent.click(screen.getByRole('button', { name: /dismiss/i }))
      await waitFor(() => expect(screen.getByRole('button', { name: /undo/i })).toBeTruthy())
    ...
    it('reports a failure instead of pretending it worked', async () => {
      setup({ onAccept: () => Promise.reject(new Error('Category already exists.')) })

      fireEvent.click(screen.getByRole('button', { name: /add to tree/i }))

      await waitFor(() =>
        expect(screen.getByRole('alert').textContent).toContain('Category already exists.'),
      )
    })
  })
  ...
  describe('a read-only role', () => {
    ...
    it('is shown no controls at all, not disabled ones', () => {
      // A disabled button claims a permission that will never be granted.
      setup({ canManage: false })
