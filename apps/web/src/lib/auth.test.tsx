...
const get = vi.fn(async () => ({
  ...
  // The API scopes by the token's org claim; mirror that here.
  organization_id: activeOrgId,
}))

// Keep the real `ApiError` — the suspended-org check is an instanceof test.
...
describe('AuthProvider org scope', () => {
  it('discards cached tenant data and reloads the principal when the org changes', async () => {
    ...
    // Stand-in for any tenant-scoped query cached by a page.
    client.setQueryData(['reports', 'entries-summary'], { rows: ['acme'] })

    const { rerender } = render(tree(client))
    await waitFor(() => expect(screen.getByTestId('org').textContent).toBe('org_acme'))
    expect(get).toHaveBeenCalledTimes(1)

    activeOrgId = 'org_bolt'
    rerender(tree(client))

    await waitFor(() => expect(screen.getByTestId('org').textContent).toBe('org_bolt'))
...
describe('AuthProvider failure states', () => {
  ...
  it('names a suspended organization instead of a generic failure', async () => {
    renderWithError(new ApiError(403, 'failed', { detail: 'Organization is suspended' }))

    await waitFor(() => expect(screen.getByText('Organization suspended')).toBeTruthy())
  ...
  it('does not mistake an unrelated 403 for a suspension', async () => {
    renderWithError(new ApiError(403, 'failed', { detail: 'Insufficient permissions' }))
