...
def test_invoice_detail_cross_tenant_is_404(client, seed):
    # Org A caller cannot fetch Org B's invoice
...
def test_invoice_lines_uncategorized_only(client):
    ...
    body = r.json()
    assert body["total"] == 1  # Org A has 1 uncategorized + 1 ai_categorized line
...
def test_empty_tenant_returns_empty_page(client):
    # tok_empty provisions a fresh org with no companies/invoices
