...
def test_tree_belongs_to_an_organization_and_companies_point_at_it():
    ...
    with Session(e) as s:
        ...
        # The tree is the org's; the company reaches its nodes through it.
...
def test_node_stores_parentage_and_the_materialized_path():
    ...
    with Session(e) as s:
        ...
        # The path is the name of each ancestor and of the node itself.
...
def test_depth_one_node_has_only_level_1():
    """`Direct`/`Indirect` are real rows, so `level_2` cannot be required."""
    e = _seeded()
    with Session(e) as s:
