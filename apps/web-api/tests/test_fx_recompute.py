...
The case that matters: a customer switches base currency after a year of data.
Every stored figure must be rewritten — each at *its own* historical rate, not
at one rate for the lot — and nothing the ERP posted may move.
"""
...
# The rate moved between the two dates — which is the point: recomputing must
# use each row's own date, not one snapshot for the whole company.
RATES = {
...
def company(engine):
    ...
    with Session(engine) as s:
        ...
        # No accounting date: never convertible, in any base currency.
...
def test_recompute_converts_everything_convertible(session, company):
    ...
    # 2 invoices + 1 line + 1 dated entry; the dateless entry cannot convert.
...
def test_each_row_is_converted_at_its_own_date(session, company):
    ...
    # 7.46 / 1.085 in March, 7.45 / 1.20 in June — two different rates, one run.
...
def test_switching_base_currency_rewrites_the_stored_figures(session, company):
    ...
    assert march.fx_rate == Decimal("0.92165899")  # 1 / 1.085
...
def test_recompute_fills_in_what_an_outage_left_behind(session, company):
    # First pass: no rates available at all.
