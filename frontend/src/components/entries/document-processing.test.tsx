...
function setup(overrides: Partial<InvoiceDetailRead> = {}, canRetrigger = true) {
...
const action = () => screen.queryByRole('button', { name: /process document again/i })
...
describe('DocumentProcessing', () => {
  ...
  it('offers no action while a run is in flight, which the API would refuse', () => {
    // A disabled control would claim a permission; there is simply none.
  ...
  it('offers no action to a user without the role the endpoint requires', () => {
    ...
    // The state and its reason are still shown — only the action is withheld.
    expect(screen.getByText('Failed')).toBeTruthy()
  ...
  it.each<DocStatus>(['not_applicable', 'pending', 'processing', 'processed', 'failed'])(
