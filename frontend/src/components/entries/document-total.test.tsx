 ...
 *
 * A component that renders a second total whenever one exists teaches the
 * reader to ignore it; one that renders nothing when the document stated no
 * total, but the same nothing when the totals agree, loses the distinction the
 * server went to the trouble of preserving.
 */
...
describe('DocumentTotal', () => {
  ...
  it('shows nothing when the two totals agree', () => {
    render(<DocumentTotal invoice={invoice({ document_total: '90.00', totals_agree: true })} />)
  ...
  it('omits the VAT clause when the document stated no VAT', () => {
    render(
      <DocumentTotal
        invoice={invoice({ document_total: '104.85', document_tax: null, totals_agree: false })}
      />,
    )
