...
/**
 * Email addresses with their verification flow. Deliberately not a
 * `useSettingsSubmit` form: this is a two-step flow (create → confirm a code)
 * rather than a save, so it tracks its own stage.
 */
        ...
        <ul className="flex flex-col divide-y divide-border">
          {emails.map((email) => (
            <li key={email.id} className="flex flex-wrap items-center gap-3 py-3">
              <span className="min-w-0 flex-1 truncate text-sm">{email.address}</span>
              ...
              {/* The primary address is what identifies the account — never removable. */}
              {!email.primary ? (
                <Button
                  size="sm"
                  variant="ghost"
...
export function EmailsPanel({ user }: { user: ClerkUser }) {
  // The address created by `onAdd`, awaiting its code.
  ...
  return (
    <EmailsView
      ...
      onVerify={async (code) => {
        if (!pending.current) throw new Error('No address is awaiting verification.')
