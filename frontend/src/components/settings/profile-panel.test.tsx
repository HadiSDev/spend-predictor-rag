...
function renderPanel(overrides: Partial<Parameters<typeof ProfilePanel>[0]> = {}) {
...
describe('ProfilePanel', () => {
  ...
  it('enables save once a field changes, then saves', async () => {
    ...
    // Saved values are no longer dirty, so the control disables again.
  ...
  it('surfaces a failure and keeps what the user typed', async () => {
    const onSave = vi
      .fn()
      .mockRejectedValue(new ApiError(403, 'generic', { detail: 'Insufficient permissions' }))
    renderPanel({ onSave })

    fireEvent.change(screen.getByLabelText('First name'), { target: { value: 'Grace' } })
    fireEvent.click(save())

    await waitFor(() => expect(screen.getAllByText('Insufficient permissions').length).toBeGreaterThan(0))
