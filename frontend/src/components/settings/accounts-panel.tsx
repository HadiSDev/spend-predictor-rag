...
import type { ErpAccountRead, ErpAccountUpdate } from '#/lib/types'

/** Matches an account on code or name — the two things anyone searches by. */
...
export interface AccountsPanelProps {
  ...
  loading?: boolean
  canManage: boolean
  /** Null when the company has no connected integration at all. */
...
/**
 * A company's ERP chart of accounts and the two settings we own on it.
 *
...
export function AccountsPanel({
  ...
  async function toggle(account: ErpAccountRead, changes: ErpAccountUpdate) {
    ...
    try {
      ...
      // The switch is driven by server state, so a failure leaves it where it
      // was — the row just has to say why, rather than showing a setting that
      // did not persist.
      setErrors((e) => ({
        ...e,
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        {/* A callback rather than a `Link`, so this panel renders in tests
            without a RouterProvider — the same shape as every other settings
            panel. */}
        <button
          type="button"
          onClick={onBack}
        ...
        <>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              {/* Labelled with the count so a filtered view is never mistaken
                  for the whole chart. */}
              <Button
                variant="outline"
