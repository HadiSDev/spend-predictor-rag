...
/** Clerk accepts these; anything else is rejected after the upload round-trip. */
...
/** Personal identity: name plus avatar. Presentational — all IO is injected. */
export function ProfilePanel({
  ...
  // Adopt values that changed elsewhere (another tab, Clerk) unless the user is
  // mid-edit — their input always wins.
  const { firstName, lastName } = defaultValues
  const { reset, formState } = form
  React.useEffect(() => {
    if (!formState.isDirty) reset({ firstName, lastName })
  ...
  async function handleFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImageError(error instanceof Error ? error.message : 'Couldn’t upload that image.')
    } finally {
      setImageBusy(false)
    }
  }
  ...
  async function handleRemoveImage() {
    ...
    try {
      ...
      setImageError(error instanceof Error ? error.message : 'Couldn’t remove your photo.')
    } finally {
      setImageBusy(false)
    }
  }
  ...
  return (
    <SettingsCard title="Profile" description="How you appear across the workspace.">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex min-w-0 flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {imageUrl ? (
                <Button size="sm" variant="ghost" disabled={imageBusy} onClick={handleRemoveImage}>
                  Remove
                </Button>
              ) : null}
            </div>
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                ...
                name="firstName"
                rules={{ maxLength: { value: 60, message: 'Keep it under 60 characters.' } }}
                render={({ field }) => (
                  <FormItem>
