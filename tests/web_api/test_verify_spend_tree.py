"""Verifying by node: the correction that always resolves.

Typed levels that match no node produce a categorization resolving to nothing —
the silent failure the stored `spend_category_id` exists to prevent. So a
correction names a node, and the server derives the levels from it.
"""
...
def test_a_line_with_a_decision_and_no_node_reads_stale(client, seed):
    """`line_a2` is seeded categorized with no pointer — exactly the stale shape."""
    body = client.get(
...
def test_stale_is_scoped_like_every_other_filter(client, seed):
    """No company_id ⇒ the caller's active companies, not the whole table."""
