"""The spend-tree service: the invariant that keeps the two node representations agreeing.

A ``SpendCategory`` states its position twice — once as parentage
(``parent_id``/``depth``) and once as a materialized path
(``level_1..level_4``). The service is the only writer, and these tests are what
say so: after every mutation the stored path must equal the path you get by
walking parents. A drift here is invisible in the UI and wrong in every report.
"""
...
def test_ensure_default_tree_seeds_the_template(session):
    ...
    # A depth-1 node's path is its level_1 alone.
...
def test_move_rewrites_paths_and_clears_the_tail(session):
    ...
    # Moved shallower: the level below the new depth must be cleared, not stale.
...
def test_a_move_that_would_push_children_too_deep_is_rejected(session):
    ...
    session.commit()
    with pytest.raises(service.SpendTreeError):
        # Would put "Cloud" at level 4 in a 3-level tree.
