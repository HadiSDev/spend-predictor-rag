...
The rejection tests matter more than the happy path. A file with one bad row
that loads 199 of 200 categories leaves a taxonomy with an invisible hole, and
every categorization that lands in it looks like a considered decision.
"""
...
def test_import_materializes_the_implied_interior_nodes(session):
    ...
    # The file lists four leaves; the roots and mid-levels are implied.
