...
def test_echoed_deletion_is_noop(client, engine, seed, clerk_recorder):
    ...
    # Clerk echoes organization.deleted back to us
    r = post_event(client, "organization.deleted", {"id": "clerk_orgA"}, "echo-1")
    assert r.status_code == 200
    with Session(engine) as s:
        org = s.get(Organization, seed["org_a"])
        assert org.status == "suspended" and org.suspended_at == before  # unchanged


# -- ClerkClient gating (7.6) — no network calls ------------------------------
...
def test_client_skips_when_no_clerk_org_id():
    ...
    assert c.delete_organization("") is False  # nothing to delete, no call
