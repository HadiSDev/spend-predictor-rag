...
def test_a_member_can_read_the_trees(client, seed, tree):
    """Reads are open: a reviewer picking a category needs the tree."""
...
def test_detail_returns_a_renderable_hierarchy(client, tree):
    ...
    # Every parent named is present, and shallower nodes come first — so a
    # client can build the tree in a single pass.
...
def test_another_organizations_tree_is_not_found(client, tree):
    """404, never 403: the existence of another tenant's tree is not ours to disclose."""
...
def test_cloning_another_organizations_tree_is_not_found(client, engine, seed, tree):
    """Org A's admin may clone — but not Org B's tree, which it cannot see."""
    with Session(engine) as s:
...
def test_import_loads_a_csv(client, engine, seed):
    ...
    assert response.status_code == 200
    # Two leaves plus the two implied ancestors.
...
# -- The default tree, obtainable without creating a company -----------------


def test_the_default_tree_can_be_materialized_on_demand(client, engine, seed):
    """An org that predates spend trees must still be able to get the default.

    Copy-on-first-use otherwise has exactly one trigger — creating a company —
    so an existing organization would have no way to obtain it at all.
    """
    assert client.get("/api/v1/spend-trees", headers=_auth("tokA")).json() == []
...
def test_the_default_is_not_confused_with_a_tree_named_default(client, engine, seed):
    """`POST /spend-trees/default` must not be swallowed by the generic create."""
...
def test_an_unused_tree_is_deleted_outright(client, engine, seed, tree):
    """Hard, not soft: a tree owns no financial record, so a mistake is removable."""
