...
# integration needs no credentials — and therefore no encryption key.
...
def test_create_returns_and_persists_the_integration(client, engine):
    ...
    # It behaves like any other integration from here on.
...
def test_create_with_credentials_stores_them_encrypted(client, engine, monkeypatch):
    ...
    assert secret not in r.text  # the secret is never echoed
    with Session(engine) as s:
...
def test_create_rolls_the_company_back_when_the_integration_fails(client, engine, monkeypatch):
    # A credential that cannot be encrypted (no key configured) fails after the
    # company row is staged — the whole transaction must unwind.
...
def test_recompute_reports_what_it_did(client, seed):
    ...
    assert body["base_currency"] == "DKK"
    # FX is off by default, so nothing converts — but the rows are still counted
    # rather than quietly skipped. Org A seeds one invoice and two lines.
