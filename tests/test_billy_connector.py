...
Every response here is a real Billy payload with names, ids and CVR numbers
pseudonymised — so these tests fail when Billy's *shape* changes, which is the
point. Nothing touches the network: the connector's `_http` is an
`httpx.MockTransport` client.

Several assertions exist specifically because the live API contradicted its own
documentation; those are marked where they appear.
"""
import json
...
class Billy:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/transactions"):
            ...
            # Billy honours sorting (and ignores date filters — see below).
...
def test_authorize_discovers_the_organization_from_the_token(connector):
    """A Billy token is bound to one organization, so the user need not find it."""
...
def test_accounts_map_from_the_real_chart(connector):
    ...
    for code, account in by_code.items():
        ...
        # The ERP's tax rate seeds the customer's with_vat setting, once.
        assert account.with_vat is bool(source["taxRateId"])
        # natureId is already the semantic string, so no second request.
        assert account.erp_account_type == source["natureId"]
        # Billy groups are not accounts; claiming one as a parent would invent
        # a level of the chart that does not exist.
        assert account.parent_code is None


def test_the_chart_covers_both_vat_settings(connector):
    """Guards the fixture itself: the assertion above is vacuous without variety."""
    assert {a.with_vat for a in connector.fetch_accounts()} == {True, False}


def test_an_archived_account_is_inactive(billy, monkeypatch):
    """The live organization archives nothing, so this one case is constructed
    rather than captured — the mapping still has to be right the first time a
    customer archives an account."""
    chart = load("accounts")
...
def test_the_blank_voucher_numbers_do_not_collide(connector):
    """Half a real ledger shares `voucherNo` of null or "" — keying on it would
    have merged unrelated vouchers on the first sync."""
    ...
    # Each blank-numbered live transaction still has its own group.
...
def test_no_entry_claims_an_invoice_line(connector):
    """A Billy posting references its transaction, never a bill line."""
...
def test_a_posting_on_an_unknown_account_is_dropped(billy, caplog):
    ...
    # Dropped rather than stored under a blank code, which would be unfilterable
    # and would pollute every per-account total.
...
def test_sales_invoices_are_not_returned(connector):
    """Revenue is not spend, the same reason payments are excluded downstream."""
...
def test_the_originator_kind_is_read_from_the_reference_not_the_type_field(connector):
    """`originatorType` is null on every real row; reading it would send every
    transaction to the fallback type."""
    source = load("transactions")["transactions"]
...
def test_a_bill_is_reachable_from_exactly_one_voucher(connector):
    """Counting voided transactions would present one bill under two voucher
    ids and duplicate the invoice."""
    entries = connector.fetch_entries()
...
def test_the_scan_sorts_and_never_sends_a_date_filter(connector, billy):
    """Billy accepts `minEntryDate` on this endpoint and ignores it, so sending
    one would silently re-fetch the whole ledger every sync."""
...
def test_a_blank_supplier_invoice_number_falls_back_to_the_bill_id(connector):
    """The first real bill observed had no `suppliersInvoiceNo` *and* a blank
    `voucherNo`, so this fallback carries real data, not a hypothetical."""
...
def test_the_scan_reports_the_attached_document(connector):
    """Without this the runner writes no `File` row, and the document endpoint
    404s on an invoice whose document Billy is perfectly willing to serve."""
    attachment = load("attachments")["attachments"][0]
    scan = connector.fetch_invoice_scan(_bill_voucher(connector))
    assert scan.file_ref == attachment["fileId"]
    # The real name, not the runner's "scan.pdf" placeholder — which is an
    # outright lie for the receipt photos Billy customers attach.
    assert scan.file_name == load("file")["file"]["fileName"]


def test_an_unreadable_file_record_still_records_the_document(billy):
    """A name is not worth failing a sync over; the document route resolves the
    record again when someone opens the file."""
...
def test_an_attachment_owned_by_something_other_than_this_bill_is_not_claimed(billy):
    """`ownerReference` is a `kind:id` string across every owner kind, so a
    prefix match would hand one bill an invoice belonging to another."""
...
def test_the_lowest_priority_attachment_is_the_invoice(billy):
    """A bill can carry several attachments; Billy's own `priority` orders them."""
...
def test_the_attachment_listing_is_fetched_once(connector, billy):
    """It is org-wide and unfilterable, so a fetch per bill would be one whole
    listing per invoice for the entire sync."""
    voucher = _bill_voucher(connector)
    connector.fetch_invoice_scan(voucher)
    ...
    # And the file record is shared between the scan and the document, rather
    # than fetched once for the name and again for the download URL.
...
def test_the_document_is_fetched_without_our_credential(connector, billy):
    """Billy's object storage *accepts* the access token, so nothing would fail
    to reveal the credential being handed to a third party."""
    voucher = _bill_voucher(connector)
    ...
    # Billy's own record names the file; the download carries no disposition.
...
def test_an_image_attachment_keeps_its_own_media_type(billy):
    """Not every attachment is a scan — customers attach phone photos and
    screenshots, and octet-stream makes a browser download rather than show it."""
    file_body = load("file")
...
def test_the_supplier_name_is_resolved_from_the_contact(connector):
    """Real bills carry `contactName: null`, so without the contact lookup the
    invoice would land in the ledger with a blank vendor."""
