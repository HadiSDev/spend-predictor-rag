# billy-erp-connector Specification

## Purpose

Billy (billy.dk) is the first real ERP this product connects to, and it is the
proof that the connector seam works against an accounting system we do not
control. This capability defines how Billy API v2 maps onto the connector
contract: how a customer's revocable access token authenticates and discovers
its own organization, how Billy's ledger — transactions carrying postings, with
a supplier bill behind some of them — becomes vouchers, entries, invoices and
invoice scans, and which of Billy's behaviours we rely on in spite of its
documentation.

Four of those behaviours contradict Billy's published API and are therefore
stated here as requirements rather than left to the code to imply: the
originator of a transaction is readable only from a `"kind:id"` reference string
because the documented type field is always null; the transaction listing
accepts date filters and silently ignores them, so the watermark has to be an
early stop on a sorted scan; document downloads live on object storage that
*accepts* the Billy credential, so withholding it is a rule no failure would
ever enforce for us; and a voided transaction survives as a pair whose halves
