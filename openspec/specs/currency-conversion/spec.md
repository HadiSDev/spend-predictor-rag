...
Money enters the system in whatever currency the ERP posted it in, and every
monetary aggregate groups by currency so amounts are never summed across them.
That guard is correct, and it is also why a company buying in DKK, EUR and USD
never sees one total. This capability defines the base currency a company
reports in, how historical daily reference rates are sourced and cached, which
date a row is converted at, what happens when a rate cannot be obtained, and how
stored conversions are recomputed.

The rule the whole capability turns on: a row is converted at the rate in force
on **its own transaction date**, the as-posted currency and amount are kept
untouched as evidence, and the rate used is stored beside the result so any
figure can be re-derived and defended.
