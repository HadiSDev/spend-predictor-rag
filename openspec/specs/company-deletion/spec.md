# company-deletion Specification

## Purpose

Removing a company and everything scoped to it, as the irreversible counterpart
to deactivation.

Deactivation is the default and destroys nothing: a company that owns a ledger is
retired from the listings, keeps every record, stays reachable by id, and can be
brought back. This capability covers the other case — a company that should not
exist at all, created by mistake, a trial that never synced, a test tenant, or
one a customer has asked to have removed — where leaving the records in place is
itself the wrong outcome.

It defines who may do it (a platform system admin, never an organization admin,
because nothing can undo it), how the confirmation gate works (refused with the
counts and the accounting date span until the caller confirms, so the scale of
what would be lost is visible before the fact, and a refusal changes nothing),
what is destroyed (the company, its ERP integrations and credentials, accounts
and sync state, files, invoices and invoice lines, ERP entries, recommendations,
and the audit rows describing the invoices and lines that went with them), and
what deliberately survives — the global `Vendor` catalog, which is shared across
tenants and never one company's to remove, and the organization's spend trees,
which several companies may share.

Afterwards the company is unreachable rather than hidden: absent from every
listing including `?include_inactive`, `404` on every route that resolves a
company by id, and absent from the reports — which is exactly the distinction
from a deactivated company, whose history still loads.
