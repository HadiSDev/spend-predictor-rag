# web-api-admin-panel Specification

## Purpose

The API surface a browser admin panel needs beyond the domain endpoints
(companies, invoices, entries, integrations, reports): cross-origin access so a
browser SPA can call the API at all, a current-user endpoint so the panel can
render role-aware UI, an organization member directory, and a tenant-scoped
vendor list. Every endpoint here is read-only and takes its tenant scope from
the shared dependency — this capability adds no authorization model of its own
(see `web-api-auth` and `web-api-authorization`).
