...
- [x] 4.1 Add the missing leaves to `web_api/spend_trees/template.py`: `Travel & Entertainment > Ground Transport` (rail, bus, taxi), `Financial > Bank & Payment Fees`, `Financial > Insurance`, `Indirect > Subscriptions & Memberships`. Keep three levels and the closed `Direct`/`Indirect` root. Bump `TEMPLATE_VERSION`.
- [x] 4.2 Test that a version bump does not touch an existing organization's copy — no nodes added, removed or renamed, `template_version` unchanged, no line made stale.
- [x] 4.3 Add a `SpendCategorySuggestion` model (tree id, proposed name/description, `parent_id`, rationale, evidence line ids, state `pending|accepted|dismissed`) with an Alembic migration.
