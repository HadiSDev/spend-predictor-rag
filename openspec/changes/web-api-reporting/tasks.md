...
- [ ] 1.5 `spend_by_vendor` — `Invoice` joined to `Vendor`; group by
  `(vendor_id, vendor_name, currency)`; sum(total), count; invoice-date range

## 2. Schemas & router

- [ ] 2.1 Add response schemas in `schemas.py` (row models + `{rows: [...]}`
  envelopes) for the four reports
- [ ] 2.2 Add `web_api/routers/reports.py` (`prefix=/api/v1`, tag `reports`) with
  the four `GET /reports/*` endpoints: `tenant_scope` + `resolve_company_ids`,
  optional `company_id`/`entry_type`/`from`/`to`/`level` query params
- [ ] 2.3 Register the router in `web_api/app.py`

## 3. Tests

- [ ] 3.1 `tests/web_api/test_reports.py`: seed entries/lines/invoices/vendors
  across two orgs; assert org scoping + foreign `company_id` → 404 + empty scope
- [ ] 3.2 entries-summary/by-account: debit/credit/net/count correct; currencies
  not combined; `entry_type` and `accounting_date` range filters
- [ ] 3.3 spend-by-category: only `ai_categorized`/`verified` lines counted;
  `level_2` vs `level_2`+`level_3` granularity
- [ ] 3.4 spend-by-vendor: totals + count per vendor with name; date range
- [ ] 3.5 `uv run pytest` — full suite green

## 4. Docs

- [ ] 4.1 `CLAUDE.md`: add the `/reports/*` endpoints and note reporting lives in
