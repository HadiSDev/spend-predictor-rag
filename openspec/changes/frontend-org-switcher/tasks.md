...
- [x] 1.1 Create `frontend/src/components/ui/combobox.tsx` wrapping Base UI's `Combobox` parts: `Combobox` (Root), `ComboboxInput`, `ComboboxTrigger`, `ComboboxValue`, `ComboboxIcon`, `ComboboxClear`
...
- [x] 1.7 Export the Combobox parts from `frontend/src/components/ui/index.ts`

## 2. Combobox verification

- [x] 2.1 Add Combobox tests to `frontend/src/components/ui/ui.test.tsx`: typing filters options case-insensitively; selecting an option closes the popup, updates the trigger text, and fires `onValueChange` once
...
- [x] 2.4 Add a Combobox section to the `/ui` kitchen-sink route (`frontend/src/routes/ui.tsx`) showing default, grouped, disabled, empty, and loading states

## 3. Organization switching state

- [x] 3.1 In `frontend/src/lib/auth.tsx`, add an effect in `AuthProvider` that observes `useAuth().orgId` and, when it changes from a previously seen value, calls `queryClient.clear()`
...
- [x] 3.4 Review `frontend/src/routes/_authed.tsx` so the pending-org activation and the switcher read memberships through the same `useOrganizationList` options (no second, differently-configured listing)

## 4. Org switcher component

- [x] 4.1 Create `frontend/src/components/org-switcher.tsx` reading memberships from `useOrganizationList({ userMemberships: { pageSize: 50 } })` and the active org from `useAuth`/`useOrganization`
...
- [x] 5.1 Place `OrgSwitcher` in the sidebar header of the app shell in `frontend/src/routes/_authed/index.tsx`, beside the product mark
...
- [x] 7.1 Run `pnpm test`, `pnpm lint`, and `pnpm build` in `frontend/` and fix fallout
