...
Shared provisioning (used by both the JIT auth path and the ``organization.created``
webhook) plus the webhook event handlers that apply Clerk organization /
membership / user events to our domain. Deletion is soft-suspend, never a
cascade delete.
"""
...
from .auth import ClerkPrincipal, map_role
from .db.models import File, Organization, User

logger = logging.getLogger("web_api.clerk_sync")
...
# -- Provisioning (JIT auth + organization.created webhook share this) --------
...
def get_or_create_organization(
    ...
    if org is None:
        ...
        try:
            session.commit()
        except IntegrityError:  # concurrent create — reuse the winner
            session.rollback()
            org = session.exec(
                select(Organization).where(Organization.clerk_org_id == clerk_org_id)
            ).one()
...
def _upsert_user(
    ...
    *,
    ...
    ``is_system_admin=None`` leaves the existing flag untouched (membership
    events don't carry it); on create it defaults to ``False``.
    """
...
def provision_user(session: Session, principal: ClerkPrincipal) -> User:
    ...
    Does not resurrect a suspended organization (status is left as-is).
    """
...
# -- Mutations driven by webhook events --------------------------------------
...
def suspend_organization(session: Session, clerk_org_id: str) -> Organization | None:
    """Soft-suspend an org (idempotent). Retains all child data."""
    org = session.exec(
        select(Organization).where(Organization.clerk_org_id == clerk_org_id)
    ).first()
    if org is None:
...
def revoke_user(session: Session, clerk_user_id: str) -> None:
    ...
    if user is None:
        return
    # Null nullable back-references before deleting the user.
...
def _apply_membership(session: Session, data: dict) -> None:
    ...
    _upsert_user(
        ...
        is_system_admin=None,  # membership events don't carry the platform flag
    )
...
def handle_clerk_event(session: Session, event_type: str, data: dict) -> None:
    """Apply a verified Clerk webhook event to the domain. Idempotent per type."""
    if event_type == "organization.created":
        org = get_or_create_organization(
            session, clerk_org_id=data.get("id"), name=data.get("name"), slug=data.get("slug")
        )
        # An explicit create/restore reactivates a previously suspended org
        # (JIT provisioning never does — only this webhook path).
